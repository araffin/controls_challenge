...
def create_report(test, baseline, sample_rollouts, costs, num_segs):
  ...
  <html>
    ...
    for ax, rollout in zip(axs, sample_rollouts):
        ...
        ax.plot(
            rollout["baseline_controller_lataccel"], label="Baseline Controller Lateral Acceleration", color=COLORS["baseline"]
        )
        ax.set_xlabel("Step")
...
if __name__ == "__main__":
    ...
    for _d, data_file in enumerate(tqdm(files[:SAMPLE_ROLLOUTS], total=SAMPLE_ROLLOUTS)):
        ...
        baseline_cost, baseline_target_lataccel, baseline_current_lataccel = run_rollout(
    ...
    for controller_cat, controller_type in [("baseline", args.baseline_controller), ("test", args.test_controller)]:
        ...
        if controller_cat == "baseline" or True:
